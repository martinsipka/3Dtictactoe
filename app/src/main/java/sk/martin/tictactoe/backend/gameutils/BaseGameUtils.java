...
public class BaseGameUtils {
        ...
        switch (actResp) {
            ...
            case GamesActivityResultCodes.RESULT_APP_MISCONFIGURED:
                errorDialog = makeSimpleDialog(activity,
                        activity.getString(R.string.play_bot));
                break;
    ...
    public static byte[] createByteArray(byte[] playBoard, int[][][] board, int offset){

        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                for(int k = 0; k < 4; k++){
                    playBoard[i+(j*4)+k*16 + offset] = (byte) board[i][j][k];
                }
            }
        }
    ...
    public static int[][][] createIntArray(byte[] board, int offset){

        int[][][] playBoard = new int[4][4][4];
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                for(int k = 0; k < 4; k++){
                    playBoard[i][j][k] = (int) board[i+(j*4)+k*16 + offset];
                }
            }
        }
    ...
    public static void updateBoard(int[][][] playBoard, int[][][] newBoard){
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                for(int k = 0; k < 4; k++){
                    playBoard[i][j][k] = newBoard[i][j][k];
                }
            }
        }
    }

    public static Move findNew(int[][][] playBoard, int[][][] newBoard, int turn){
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                for(int k = 0; k < 4; k++){
        ...
        throw new RuntimeException("no move changed");
    }

    public static void longToBytes(long l, byte[] result, int offset) {
        for (int i = 7; i >= 0; i--) {
